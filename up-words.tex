...
\end{proof}
% 
A concluding remark: this theorem can be rephrased as ``every $\omega$-regular nominal language is characterized by its finitely-supported fragment'' (recall that the support of $uv^\omega$ just contains the symbols in $u$ and $v$). This is a very appealing result, as finite support has a central role in the nominal setting.\todo{Marketing: che ne pensi? :D}

%, because it implies that every nominal language, even those containing infinitely-supported words, are characterized by their 
