...
The intuition for the following lemma is that names assigned to $T$ are eventually forgotten.
...
\item Let $\pi \colon I \to I$ be the function $\restr{\widehat{\sigma}}{I}$ with its codomain restricted to $I$. Then $\pi$ is an element of the symmetric group on $I$, so it has an order $\theta$, that is a positive integer such that $\pi^\theta = id_I$\todosm{Dovrebbe funzionare sempre con $\theta = |I|!$, cioè con l'ordine del gruppo}. Hence $\restr{ \rho_\theta }{ I } =\restr{ \rho }{I} \circ \pi^\theta = \restr{ \rho }{I}$.
...
Take any string $\alpha$ accepted by $A$ and let $I = Inf(q_0,\alpha)$, so $I \in \acc$. A path spelling $\alpha$ in the configuration graph must \todo{Spiegare meglio perchè ``must''?} begin with
\[
