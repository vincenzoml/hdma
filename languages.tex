...
\begin{center}
\subfigure[]{
\begin{tikzpicture}[->,shorten >=1pt,auto,node distance=15ex,semithick,initial text={},scale=0.7, every node/.style={scale=0.7}]  
  \node[state] (q0)               {$q_0$};
...
\subfigure[]{
{\centering
\begin{tikzpicture}[->,shorten >=1pt,node distance=14ex,auto,semithick,initial text={},scale=0.7, every node/.style={scale=0.7}]
...
\label{fig:automata}
\caption{Some automata, together with their accepting conditions. \subref{fig:example-session} is the nDMA of \cref{exa:session} and \subref{fig:hd-of-example} is the corresponding \hdma; \subref{fig:upwords-ex} is an example \hdma\, where we have omitted the accepting condition and unrelevant transitions, all assumed to end up in a sink state without registers; \subref{fig:hd-names-omega} is the \hdma\ recognizing $\names^\omega$.}
\end{figure}
