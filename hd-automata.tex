...
\begin{remark}
To keep the notation lightweight, we do not introduce the notion of a \emph{symmetry} attached to states of an \hdma. It is well known (see \cite{PistoreThesis?}) that this would be required for minimization. However, notice that minimization of (classical) Muller automata up-to language equivalence is not possible. A solution is to represent languages using two-sorted structures \cite{CV12}. Applying the same idea to hDMAs is not straightforward, and requires further investigation (more about this in the conclusions), thus symmetry would not be used in this work anyway. \todo{more about this in the conclusions!}
...
The \emph{configuration graph} of $A$ is a transition relation  over triples \todo{is a graph with edges of the form ... } 
...
\end{definition}
% 
\todo{Throughout the paper, change all ref to cref for consistency}
...
%
%
Differently from nDMAs, \hdmas\ have finite states. The configuration graph is deterministic, even if still infinite-state and infinite-branching.
Finite representations are useful for effective operations on languages, as we shall see later. The similarity between configuration graphs of \hdmas, and nDMAs, is deeper than expected, as stated in the following propositions. These are derived from the equivalence results in \cite{GADDUCCIETAL,STATON}, adapted to the setting of nominal Muller automata; we omit information about symmetry; the equivalence being up-to accepted languages, this does not harm (but it matters for minimization; see Remark \ref{rem:no-symmetry}).
